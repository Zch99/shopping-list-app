...
export default function ShoppingListApp() {
  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          {editingListName ? (
            ...
            <h1 className="text-2xl font-bold cursor-pointer" onClick={() => setEditingListName(true)}>
          ...
          <SelectContent className="bg-white rounded-md shadow-lg">
            {lists.map((list, i) => (
              <div key={i} className="flex items-center justify-between px-2 py-1 hover:bg-gray-100">
                <SelectItem value={i.toString()} className="flex-1">
                  {list.name}
                </SelectItem>
                {lists.length > 1 && (
                  <button
      ...
      <div className="text-right font-semibold text-lg">Total: €{total.toFixed(2).replace('.', ',')}</div>
